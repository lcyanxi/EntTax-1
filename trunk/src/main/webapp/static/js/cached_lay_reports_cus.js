...
;(function($,win,doc,undefined){
    var blacklist = ['duration','frequency','depth','interval','devices_devices','devices_resolutions','devices_versions','network_access','network_carriers','location_cities','location_countries'];
    win.um = win.um || {};
    win.um.userCustomDate = {
        localCache : '',//localStorage.setItem('localCache',''),
        setDate: function(array){
            var date = {};
            if(array.constructor === Array && array.length == 2){
                array.sort();
                localStorage.setItem('localCacheDate',JSON.stringify(array));
                localStorage.setItem('localCacheDateExpireTime',new Date().getDate());
            }
        },
        getDate: function(n){
            var days = n || -7;
            if(localStorage['localCacheDate'] != undefined){
                var array = new Array(localStorage['localCacheDate']);
    ...
    function loadLocalDate(){
        ...
        var val = $('#action_stats').val();
        var flag = $.inArray(val,blacklist);
        if(flag == -1 && expire == localStorage['localCacheDateExpireTime']){
            var date = um.userCustomDate.getDate();
...
window.global.action_stats = UMENG.actionStats = $('#action_stats').val();
window.global.time_unit = '';
if($('#app_id').length>0){
    window.global.appid = $('#app_id').attr('attr-id');
}else{
	window.global.appid ="1";
}
...
;(function($){
    $('body').delegate('a[action-frame]','mouseover',function(){
        if($('body').width()-$(this).offset().left < 400){
            $('#'+$(this).attr('action-frame')).fadeIn().css({right:$('body').width()-$(this).offset().left+10,top:$(this).offset().top-12});
        }else{
...
function initTimeUnit(e,n){
    ...
    if( time_unit_component != undefined ){
        ...
        $.publish('/'+UMENG.actionStats,global.time_unit);
    }
};
	...
	return {
		...
		init : function(target,options){
			...
			var defaults = {
				...
				panelTempl : '<div class="filterpanel" style="display:none;"><input type="text" class="input" placeholder="搜索全部"/><ul class="filterlist"></ul><div class="load" style="margin:10px auto;text-align:center;display:block;"><img src="/static/img/ajax-loader.gif"/></div><div class="submitpanel"><a href="#" class="submit">确定</a></div></div>'
			};
			var settings = $.extend({},defaults,options);
    ...
    $.fn.ProSelect = function(options){
        if(this.length >0){
            ...
            var defaults = {
                ...
                tempid : 'proTemp',
                inputname : ['最近60天','最近30天','最近7天','昨天','今天'],
                inputval : ['-60','-30','-7','-2','-1'],
                custom : true,
                callback : ''
            };
            if(options){
...
var Contrast = function(){
	...
	this.bindSubmit = function(settings){
      ...
      $('#'+settings.contrastTo).after('<div class="loadingChart"><img src="/static/img/ajax-loader.gif" /></div>');
...
;(function($){
 $.fn.renderTable = function(options){
   return this.each(function(){
     ...
     if( settings.data == '' ){
       loading_icon.html('<img src="/static/img/ajax-loader.gif" />').show();
       $.ajax({
         url:settings.url,
...
function render_chart(chart_id, title, data_src_url, params, force_reload, opts,callback){
  ...
  if(!params.is_compared){
    var loading_img = $("<div style='text-align:center;padding-top:150px;'><img src='/static/img/ajax-loader.gif' /></div>");
