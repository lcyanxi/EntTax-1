	...
	$.addFlex = function(t, p) {
		...
		p = $.extend({
			...
			__mw			: {
				datacol	: function(e, t, n) {
					var r = typeof e.datacol[t] == "function" ? e.datacol[t](n) : n;
		...
		var g = {
			...
			dragStart				: function(e, t, n) {
				if (e == "colresize" && p.colResize === true) {
					...
					var i = $("th:visible div:eq(" + r + ")", this.hDiv).width();
			...
			switchCol				: function(e, n) {
				$("tbody tr", t).each(function() {
					if (e > n)
						$("td:eq(" + n + ")", this).before($("td:eq(" + e + ")", this));
					else
						$("td:eq(" + n + ")", this).after($("td:eq(" + e + ")", this))
				});
				if (e > n) {
					$("tr:eq(" + n + ")", this.nDiv).before($("tr:eq(" + e + ")", this.nDiv))
				} else {
					$("tr:eq(" + n + ")", this.nDiv).after($("tr:eq(" + e + ")", this.nDiv))
				}
				...
				if (p.dataType == "json") {
					$.each(e.rows, function(e, t) {
						...
						$("thead tr:first th", g.hDiv).each(function() {
							...
							if (typeof t.cell == "undefined") {
								...
								e.innerHTML = p.__mw.datacol(p, $(this).attr("abbr"), i)
							}
							var s = e.innerHTML.indexOf("<BGCOLOR=");
							if (s > 0) {
								$(e).css("background", text.substr(s + 7, 7))
							}
					...
					$("rows row", e).each(function() {
						...
						$("thead tr:first th", g.hDiv).each(function() {
							...
							if (i > 0) {
								$(t).css("background", r.substr(i + 7, 7))
							}
							t.innerHTML = p.__mw.datacol(p, $(this).attr("abbr"), r);
			...
			addTitleToCell			: function(e) {
				if (p.addTitleToCell) {
					var t = $("<span />").css("display", "none"), n = e instanceof jQuery ? e : $(e), r = n.outerWidth(), i = 0;
			...
			autoResizeColumn		: function(e) {
				...
				var t = $("div", this.cDrag).index(e), n = $("th:visible div:eq(" + t + ")", this.hDiv), r = parseInt(e.style.left, 10), i = n.width(), s = 0, o = 0, u = $("<span />");
		...
		$("thead tr:first th", g.hDiv).each(function() {
			...
			$(this).empty().append(e).removeAttr("width").mousedown(function(e) {
				...
				if (g.colCopy) {
					...
					var r = parseInt($("div:eq(" + n + ")", g.cDrag).css("left"), 10);
					var i = jQuery(g.nBtn).outerWidth();
					var s = r - i + Math.floor(p.cgwidth / 2);
					$(g.nDiv).hide();
					$(g.nBtn).hide();
					$(g.nBtn).css({
						left	: s,
					...
					var o = parseInt($(g.nDiv).width(), 10);
					$(g.nDiv).css({
						top	: g.bDiv.offsetTop
					});
					if (s + o > $(g.gDiv).width()) {
						$(g.nDiv).css("left", r - o + 1)
					} else {
						$(g.nDiv).css("left", s)
					}
		...
		if (p.usepager) {
			...
			if (p.searchitems) {
				...
				$(g.sDiv).append("<div class='sDiv2'>" + p.findtext + " <input type='text' value='" + p.query + "' size='30' name='q' class='qsbox' /> " + " <select name='qtype'>" + sopt + "</select><span style='color:red'> 提示：模糊查询</span></div>");
