				...
				<div class="bd3">
					...
					<div class="contentCol">
						...
						<div class="filterPanel">
							焦点图id:<input type="text" onblur="window.global.renderPage();"  style="width: 30px" name="focusId"
								id="focusId" value="${focusId}" />焦点图类型:&nbsp;&nbsp; <select
								id="type" name="type" onchange="window.global.renderPage();">
								<option <c:if test="${type == 'INDEX'}">selected</c:if> value="INDEX">首页</option>
								<option <c:if test="${type == 'HUODONG'}">selected</c:if> value="HUODONG">活动页</option>
							</select>&nbsp;&nbsp;&nbsp;
							<!--  
							<ul class="filteritems borders clearfix"
								style="display: compact;">
								<li class="items itemsfirst  "><span class="ell"
									id="filter-focustype">焦点图类型 </span></li>
								<li class="items  itemslast"><span class="ell"
									id="filter-channel">渠道</span></li>
							</ul>
							-->
			...
			<div class="wrap-table">
				<div class="mod mod1 parent-table" id="active_user-detail-table">
					<div class="mod-header radius">
						<h2>
							焦点图数据统计 <a class="icon help poptips"
...
<script type="text/javascript">
	...
	window.global.renderPage = function() {
		...
		if (focusId != "" && (startDateShow != $("#startDate").val() || endDateShow != $("#endDate").val() || focusId != "${focusId}" || type != "${type}")) {
			strurl = "/hiveadmin/preFocusList.do?startDate=" + startDateShow + "&endDate=" + endDateShow + "&plat=" + plat + "&type=" + type + "&focusId=" + focusId;
