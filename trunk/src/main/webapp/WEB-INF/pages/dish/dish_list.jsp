						...
						<div class="mod-select">
							<div class="tabpanel">
								<ul id="toptab" class="borders">
									<li class=<c:if test="${type == 'source'}"> on</c:if>><a href="/dish/list.do?startDate=${startDate}&endDate=${endDate}&type=source">作品统计</a></li>
									<li class=<c:if test="${type == 'week'}"> on</c:if>><a href="/dish/weeklist.do?startDate=${startDate}&endDate=${endDate}&type=week">周作品统计</a></li>
									<li class=<c:if test="${type == 'month'}"> on</c:if>><a href="/dish/monthlist.do?startDate=${startDate}&endDate=${endDate}&type=month">月作品统计</a></li>
								</ul>
							</div>
						</div>
					</div>
						...
						<table class="data-load" width="<c:out value="${tableLine.size() * 120 + 230}"/>" border="0" cellspacing="0">
							<thead>
								<tr>
									...
									<th>来源名称</th>
									<%
										Map<String, Object> rowMapDishs =  (Map<String, Object>)request.getSession().getAttribute("rowMapDishs");
							...
							<tbody id="data-list">
								...
								%>
								<tr>
									<td width="80"><%=column %></td>
									<%
										String tagname = (String)rowMapTagName.get(column);
										if(tagname == null){ tagname = "";}
									%>
									<td width="150"><%=tagname%></td>
								<%
								
									for( String line : tableLine){
										String rownum = (String)rowMapDishs.get(column+"-"+line);
										if(rownum == null){ rownum = "0";}
									%>
									<td width="100"><%=rownum %></td>
									<%
									}
								%>
								</tr>
