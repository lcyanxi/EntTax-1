			...
			<div class="wrap-table">
				<div class="mod mod1 parent-table" id="active_user-detail-table">
					<div class="mod-header radius">
						<h2>
							电商来源统计 <a class="icon help poptips"
						...
						<table class="data-load " width="100%" border="0" cellspacing="0">
							...
							<tbody id="data-list">
								<c:forEach items="${rowlst}" var="appdau" varStatus="ast">
									...
									<tr>
										<td><c:out value="${appdau.source}" /></td>
										<td><c:out value="${appdau.view_tuan_lists}" /></td>
										<td><c:out value="${appdau.view_tuan_detail}" /></td>
										<td><c:out value="${appdau.orders}" /></td>
										<td><c:out value="${appdau.pays}" /></td>
										<td><c:out value="${appdau.orderRate}" />%</td>
										<td><c:out value="${appdau.payRate}" />%</td>
					...
					function() {
						...
						global.renderPage = function() {
							...
							if (startDateShow != $("#startDate").val()
								...
								window.location.href = "/mall/source/list.do?startDate="
										+ startDateShow
										+ "&endDate="
										+ endDateShow + "&app=" + app;
							}
