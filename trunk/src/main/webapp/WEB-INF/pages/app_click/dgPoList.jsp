							...
							<div class="tabpanel">
								<ul id="toptab" class="borders">
									<li class=<c:if test="${page == '1'}"> on</c:if>><a
										href="/appclick/dglist.do?startDate=${startDate}&endDate=${endDate}&page=${page}">返回</a></li>
								</ul>
							</div>
						</div>
					</div>
							...
							<tbody id="data-list">
								<c:forEach items="${rowlst}" var="appclick">
									<tr>
										...
										<c:set var="key" value="${appclick.page}_${appclick.view}_${appclick.position}"></c:set>
										<td title="${key}" val="<c:out value="${appclick.view}" /> ">
											<c:out value="${mapClickDict[key]}" />
										</td>
					...
					function() {
						...
						global.renderPage = function() {
							...
							if (startDateShow != $("#startDate").val()
								...
								window.location.href = "/appclick/dgpolist.do?startDate="
										+ startDateShow
										+ "&endDate="
										+ endDateShow
										+ "&page="
