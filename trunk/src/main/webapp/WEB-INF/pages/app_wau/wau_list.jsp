						...
						<div class="mod-select">
							<div class="tabpanel">
								<ul id="toptab" class="borders">
									<li class=<c:if test="${tab == 'WAU'}"> on</c:if>><a
										href="/appwau/waulist.do?startDate=${startDate}&endDate=${endDate}&app=3&tab=WAU">APP周活统计</a></li>
								</ul>
							</div>
						</div>
					</div>
				...
				<div class="mod mod1 parent-table" id="active_user-detail-table">
					...
					<div class="mod-body " id="data-load">
						<table class="data-load " width="100%" border="0" cellspacing="0">
							...
							<tbody id="data-list">
								<c:forEach items="${rowlst}" var="appwau">
									<tr>
										<td><c:out value="${appwau.app}" /></td>
										<td><c:out value="${appwau.uid}" /></td> 
										<td><c:out value="${appwau.statdate}" /></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
			...
			function() {
				...
				global.renderPage = function() {
					...
					if (startDateShow != $("#startDate").val()
						...
						window.location.href = "/appwau/waulist.do?startDate="
