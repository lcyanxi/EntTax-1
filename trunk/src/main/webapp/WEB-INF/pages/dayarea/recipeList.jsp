						...
						<div class="mod-select">
							<div class="tabpanel">
								<ul id="toptab" class="borders">
									<li class=<c:if test="${source == 0}"> on</c:if>><a href="/dayArea/recipeList.do?startDate=${startDate}&endDate=${endDate}&source=0">网站</a></li>
									<li class=<c:if test="${source == 3}"> on</c:if>><a href="/dayArea/recipeList.do?startDate=${startDate}&endDate=${endDate}&source=3">IOS豆果美食</a></li>
									<li class=<c:if test="${source == 4}"> on</c:if>><a href="/dayArea/recipeList.do?startDate=${startDate}&endDate=${endDate}&source=4">Andriod豆果美食</a></li>
								</ul>
							</div>
						</div>
					</div>
			...
			<div class="wrap-table">
				<div class="mod mod1 parent-table" id="active_user-detail-table">
					<div class="mod-header radius">
						<h2>
							菜谱发布统计<a class="icon help poptips" action-frame="tip_activeTable"
								title=""></a>
						</h2>
...
$(document).ready(function(){
	...
	global.renderPage = function() {
		...
		{
			window.location.href="/dayArea/recipeList.do?startDate="+startDateShow+"&endDate="+endDateShow+"&source="+source;
		}		
	}
});
</script>
