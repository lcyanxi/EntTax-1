						...
						<div class="mod-select">
							<div class="tabpanel">
								<ul id="toptab" class="borders">
									<li class=on><a
										href="/user/behavior/list.do?startDate=${startDate}&endDate=${endDate}&app=${app}&qtype=${qtype}">返回</a></li>
								</ul>
							</div>
						</div>
					</div>
			...
			<div class="wrap-table">
				<div class="mod mod1 parent-table" id="active_user-detail-table">
					<div class="mod-header radius">
						<h2>
							Keyword统计 <a class="icon help poptips" action-frame="tip_activeTable"
								title=""></a>
						</h2>
					...
					<div class="mod-body " id="data-load">
						<table class="data-load " width="100%" border="0" cellspacing="0">
							...
							<tbody id="data-list">
								<c:forEach items="${rowlst}" var="userbehavior">
									<tr>
										<td><c:out value="${userbehavior.statdate}" /></td>
										<td><c:out value="${userbehavior.keyword}" /></td>
										<td><c:out value="${userbehavior.counts}" /></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
					...
					function() {
						...
						global.renderPage = function() {
							...
							if (startDateShow != $("#startDate").val()
								...
								window.location.href = "/user/behavior/keyword/detaillist.do?startDate="
										+ startDateShow
										+ "&endDate="
										+ endDateShow 
										+ "&app=" + app
