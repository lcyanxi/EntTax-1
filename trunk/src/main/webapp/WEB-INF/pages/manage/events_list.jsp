...
<html>
			...
			<form id="userform">
				<table>
					<tr>
						...
						<td>
							<input type="text" name="event_name" id="event_name" style="width:250px" />
						</td>
					</tr>
					<tr>
						<td>
							<label for="event_code">事件代码</label>
						</td>
						<td>
							<input type="text" name="event_code" id="event_code" style="width:250px" />
						</td>
					</tr>
					<tr>
						<td>
					...
					<tr>
						<td>
							<label for="is_mult">多平台</label>
						</td>
						<td>
							<select id="is_mult" name="is_mult" style="width:250px">
	...
	<script type="text/javascript">
		$(".mytable-class").flexigrid({
			url : '/manage/events/list.do',
			method:'POST',
			dataType : 'json',
			colModel : [ 
						 { display : '事件ID', name : 'id', width : 90, sortable : true, align : 'left' },
			             { display : '代码', name : 'event_code', width : 220, sortable : true, align : 'left' }, 
			             { display : '名称', name : 'event_name', width : 200, sortable : true, align : 'left' }, 
						 { display : '所属应用', name : 'app_id', width : 200, sortable : true, align : 'left', process:transform}, 
			...
			searchitems : [ 
			                { display : '事件ID', name : 'id' }, 
			                { display : '事件名称', name : 'event_name', isdefault : true},
			                { display : '事件代码', name : 'event_code'} 
			],
			sortname : "id",
			sortorder : "asc",
			usepager : true,
		...
		function handleClick(com, grid) {
			if (com == '删除') {
				...
				if (conf) {
					...
					var actionUrlStr ='/manage/events/delete.do';
					$.ajax( {
						type : "POST",
						url : actionUrlStr,
						data : "ids=" + ids,
				...
				event_code = $('.trSelected').children('td').eq(1).children('div').html();
				event_name = $('.trSelected').children('td').eq(2).children('div').html();
				app_id = $('.trSelected').children('td').eq(3).children('div').html();
				status = $('.trSelected').children('td').eq(4).children('div').html();
